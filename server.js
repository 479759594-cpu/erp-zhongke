...
function handleAPI(req, res, pathname, method) {
  ...
  if (pathname.match(/^\/api\/admin\/accounts\/[^\/]+$/) && method === 'PUT') { var aid = pathname.split('/')[3]; var admA = checkAuth(req, d); if (!admA || admA.role !== 'super_admin') return sendJSON(res, 401); parseBody(req, function(b) { for (var i = 0; i < d.adminUsers.length; i++) { if (d.adminUsers[i].id === aid) { if (b.password !== undefined) d.adminUsers[i].password = hashPassword(b.password); if (b.phone !== undefined) d.adminUsers[i].phone = b.phone; break; } } saveData(d); sendJSON(res, 200, { ok: true }); }); return true; }
